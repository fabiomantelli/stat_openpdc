...
exports.listStat = async (req, res) => {
  ...
  if (Object.keys(dataByHistorian).length === 0) {
    ...
    res.json({
      information: 'No status flags with problem.',
    });

    return;
  }
  for (const historian in dataByHistorian) {
    const sheet = workbook.addWorksheet(historian);
...
async function getStatisticFromOpenPDC(ppa, system, startTime, endTime) {
  ...
  try {
    ...
    const data = responses.map((response) => response.data.TimeSeriesDataPoints);
    return data;
  } catch (err) {
    console.error(err);
  }
